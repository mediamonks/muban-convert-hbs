...
export class TwigTranspiler implements ITranspiler {
  private buffer: any[];
  private parsed: hbs.AST.Program;
  private context: Context;
  private depth: number;

  constructor(input?: string, context?: any, depth = 0) {
    this.buffer = [];
    ...
    program.body.forEach((statement: hbs.AST.ProgramStatement) => {
      switch (statement.type) {
        ...
        case 'MustacheStatement':
          ...
          const escaped = statement.escaped ? '' : '|raw';
          ...
          switch (type) {
            case 'if': {
              ...
              if (statement.inverse) {
                // else section
                const isInverseOnlyConditional = TwigTranspiler.isOnlyCondition(statement.inverse);
                const t = new TwigTranspiler(null, this.context, this.depth);
            ...
            case 'each': {
              ...
              const t = new TwigTranspiler(null, childContext, this.depth + 1);
